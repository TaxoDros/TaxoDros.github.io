...
preston cat --data-dir "${DATA_DIR}" ${VERSION}\
 ...
 | jq '.hits.hits[]\
 | { "authors": (.metadata.creators | map(.name) | join(" | ")), "year": .metadata.publication_date, "title": .metadata.title,  "id": .metadata.alternate_identifiers[0].identifier, "doi": .links.doi}'\
 | mlr --ijsonl --otsv cat
